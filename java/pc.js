...
try {
  ...
  badFiles.forEach(f => {
    if (fs.existsSync(f) && fs.statSync(f).size === 0) {
      fs.unlinkSync(f);
      console.log('[V12-PC][CLEANUP] Đã xóa tệp rỗng 0-byte gây lỗi EOFException:', f);
    }
  });
} catch(e){}
...
app.use('/web5', (req, res, next) => {
  if (req.method === 'GET' || req.method === 'HEAD') {
    res.on('finish', () => console.log('[V12-PC][STATIC] ' + req.method + ' /web5' + req.url + ' -> ' + res.statusCode));
    try {
...
app.get('/emu/jar/:token', (req, res) => {
  res.on('finish', () => console.log('[V12-PC][JAR] ' + req.method + ' ' + req.originalUrl + ' -> ' + res.statusCode));
...
app.get('/api/launch', (req, res) => {
  ...
  if (modeParam === 'enginemode5-cheerpj-vm') {
    console.log('[V12-PC][LAUNCH] 🚀 Khởi chạy gameId=' + id + ' ở Mode 5');
...
app.use((err, req, res, next) => {
  console.log('[V12-PC][GLOBAL-ERR] ' + (err.status || 500) + ': ' + (err.message || err));
  if (err && (err.status === 416 || err.statusCode === 416)) return res.status(200).send('');
  res.status(err.status || 500).send('');
});
